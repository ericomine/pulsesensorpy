...
def loop(self):
    ...
    if self.x < self.thresh and self.N > (self.IBI/5)*3:
    ...
    if self.N > 250:                                 # avoid high frequency noise
        if self.x > self.thresh and self.Pulse == False and self.N > (self.IBI/5)*3:
            ...
            IBI = self.sampleCounter - self.lastBeatTime  # measure time between beats in mS
        ...
        for i in range(0, 8):                   # shift data in the rate array
            ...
            self.runningTotal += self.rate[i]             # add up the 9 oldest IBI values
            
        self.rate[9] = IBI                           # add the latest IBI to the rate array
